...
export async function getMerchantsByCourierServiceId(
  ...
  const payload = await requestJson(
    `/api/backend/merchant?${params}`,
    "Failed to fetch merchants for the selected courier service",
    { headers: serviceHeaders(courierServiceId, accessToken) }
  );

  return paged<Merchant>(payload, page, limit);
}
